...
export default function PurchaseOrderCreate() {
  ...
  return (
    <div data-testid="po-create-page" className="pb-24">
      <div className="flex items-center gap-2 mb-4">
        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => navg("/app/purchase-orders")}><ArrowLeft className="h-4 w-4" /></Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4 bg-slate-50 border border-slate-200 rounded-md p-3">
        <Fld label="Supplier *">
          <select value={f.supplier_id} onChange={e => pickSupplier(e.target.value)} className="w-full h-9 text-sm border border-slate-200 rounded-sm px-2 bg-white">
            <option value="">Select supplier…</option>{suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
          </select>
        </Fld>
        <Fld label="Supplier GSTIN"><Input value={f.supplier_gstin} onChange={e => set("supplier_gstin", e.target.value)} /></Fld>
        <Fld label="PO No (blank = auto)"><Input value={f.code} onChange={e => set("code", e.target.value)} placeholder="auto" /></Fld>
        <Fld label="PO Date"><Input type="date" value={f.date} onChange={e => set("date", e.target.value)} /></Fld>

        <Fld label="Delivery Date"><Input type="date" value={f.delivery_date} onChange={e => set("delivery_date", e.target.value)} /></Fld>
        <Fld label="Reference / Quote No"><Input value={f.reference} onChange={e => set("reference", e.target.value)} /></Fld>
          ...
          <tbody>
            {lines.map((l, i) => (
              <tr key={i} className="border-t border-slate-100">
                ...
                <td className="p-2"><Input list="po-item-names" value={l.description} onChange={e => pickItem(i, e.target.value)} className="h-8 min-w-[180px]" placeholder="Item or description" /></td>
      ...
      <div className="fixed bottom-0 right-0 left-0 lg:left-64 bg-white border-t border-slate-200 px-6 py-3 flex items-center justify-end gap-3 z-20">
        ...
        <Button variant="outline" className="rounded-sm" onClick={() => navg("/app/purchase-orders")}>Cancel</Button>
        <Button onClick={save} disabled={saving} className="rounded-sm bg-red-600 hover:bg-red-700"><Save className="h-4 w-4 mr-1" /> {saving ? "Saving…" : "Save Purchase Order"}</Button>
      </div>
    </div>
  );
}
