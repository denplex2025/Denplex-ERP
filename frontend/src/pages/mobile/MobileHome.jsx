  ...
  return (
    <div className="px-3 pt-3">
      ...
      <div className="flex gap-2 mb-3">
        <button
          onClick={() => setTab("txn")}
          className={`flex-1 py-2 rounded-full text-sm font-semibold border ${tab === "txn" ? "bg-red-50 border-red-500 text-red-600" : "border-slate-300 text-slate-600 bg-white"}`}
        >
          Transaction Details
        </button>
        <button
          onClick={() => setTab("party")}
          className={`flex-1 py-2 rounded-full text-sm font-semibold border ${tab === "party" ? "bg-red-50 border-red-500 text-red-600" : "border-slate-300 text-slate-600 bg-white"}`}
        >
          Party Details
        </button>
      </div>
      ...
      {!loading && tab === "txn" && (
        <div className="space-y-2.5">
          {transactions.length === 0 && <div className="text-center text-sm text-slate-400 py-10">No transactions yet.</div>}
          {transactions.slice(0, 40).map((t) => (
            <div key={`${t.kind}-${t.id}`} className="bg-white rounded-xl p-3.5 border border-slate-200">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <div className="font-semibold text-slate-900 text-sm truncate">{t.party || "—"}</div>
                  <span className={`inline-block mt-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${
                    t.color === "emerald" ? "bg-emerald-50 text-emerald-700" : t.color === "amber" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-600"
                  }`}>{t.kind.toUpperCase()} · {t.statusLabel}</span>
                </div>
                <div className="text-right flex-shrink-0">
                  {t.code && <div className="text-[11px] text-slate-400">#{t.code}</div>}
                  <div className="text-[11px] text-slate-400">{fmtDate(t.date)}</div>
                </div>
              </div>
              <div className="flex items-center justify-between mt-2.5 pt-2.5 border-t border-slate-100">
                <div><div className="text-[10px] text-slate-400 uppercase">Total</div><div className="font-semibold text-sm">{inr(t.total)}</div></div>
                {t.balance > 0.01 && <div className="text-right"><div className="text-[10px] text-slate-400 uppercase">Balance</div><div className="font-semibold text-sm text-red-600">{inr(t.balance)}</div></div>}
              </div>
            </div>
          ))}
        </div>
      ...
      {!loading && tab === "party" && (
        <div className="bg-white rounded-xl border border-slate-200 divide-y divide-slate-100">
          {parties.length === 0 && <div className="text-center text-sm text-slate-400 py-10">No parties yet.</div>}
          {parties.slice(0, 60).map((p) => (
            <div key={`${p.type}-${p.id}`} className="flex items-center justify-between px-3.5 py-3">
              ...
              <div className="text-right flex-shrink-0">
                <div className={`font-semibold text-sm ${p.balance > 0.01 ? "text-emerald-600" : p.balance < -0.01 ? "text-red-600" : "text-slate-400"}`}>
                  {inr(p.balance)}
                </div>
                <div className="text-[10px] text-slate-400">{p.balance > 0.01 ? "You'll Get" : p.balance < -0.01 ? "You'll Give" : ""}</div>
              </div>
            </div>
          ))}
        </div>
